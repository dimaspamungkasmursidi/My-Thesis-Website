<x-app-layout>
    ...
    <section class="container mx-auto sm:p-8 sm:pt-12">
        <form action="{{ route('categories.update', $category->id) }}" method="POST" class="bg-gray-800 p-8 sm:rounded-lg shadow-lg space-y-4">
            @csrf
            @method('PUT')
            <!-- Title Section -->
            <div>
                <legend class="text-xl font-semibold text-gray-200 mb-1">Edit Category</legend>
                <p class="text-sm text-gray-400">Update category details in the library</p>
            </div>
